...
parser.add_argument('--phase', metavar='p', type=str,
                    help='train or test', required=True)
args = parser.parse_args()

input_folder = args.data_dir + "/original_youtube/" + args.phase
output_folder = args.data_dir + "/spatially_cropped/" + args.phase
pickle_name = args.pickle_name

if not os.path.exists(args.data_dir + "/spatially_cropped"):
    os.mkdir(args.data_dir + "/spatially_cropped")

if not os.path.exists(args.data_dir + "/spatially_cropped/" + args.phase):
    os.mkdir(args.data_dir + "/spatially_cropped/" + args.phase)
...
for input_video in input_videos:
    ...
    video_name = base_name #base_name[base_name.find('_') + 1:]
