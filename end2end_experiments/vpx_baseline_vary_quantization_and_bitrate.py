...
parser = argparse.ArgumentParser(description='VPX Quantization or Bitrate  Variation.')
parser.add_argument('--quantization-list', type=int, nargs='+',
                    help='list of quantization levels to run on. -1 means full range', 
...
def run_experiments():
    ...
    for quantizer in args.quantization_list:
        for vpx_default_bitrate in args.vpx_default_bitrate_list:
            for vpx_min_bitrate in [50000, vpx_default_bitrate]:
                for vpx_max_bitrate in [vpx_default_bitrate, 1500000]:
                    ...
                    params['save_dir'] = f'{save_prefix}_quant{quantizer}' + \
